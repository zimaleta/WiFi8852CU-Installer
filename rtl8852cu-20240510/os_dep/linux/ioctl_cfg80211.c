...
bypass_p2p_chk:
	...
	for (i = 0; i < request->n_channels && i < RTW_CHANNEL_SCAN_AMOUNT; i++) {
		...
		parm->ch[i].band = nl80211_band_to_rtw_band(request->channels[i]->band);
	}
	parm->ch_num = i;
	...
	, struct ieee80211_channel *chan, enum nl80211_channel_type channel_type)
{
	...
	rtw_set_chbw_cmd(padapter, padapter_link,
			chan_band, chan_target, chan_width, chan_offset, RTW_CMDF_WAIT_ACK);
...
/* phl_scan will self return opch */
	...
	if (leave_op) {
		if (rtw_mi_check_status(padapter, MI_LINKED))
			set_bch_bwmode(padapter, padapter_link, u_chdef.band, u_chdef.chan, u_chdef.offset, u_chdef.bw, _FALSE);
	...
	struct protocol_cap_t *dft_proto_cap, enum nl80211_band band)
{
	...
	u8 he_6g_band_cap_ie[HE_6G_BAND_CAP_INFO_LEN] = {0};
...
#if defined(CONFIG_IEEE80211_BAND_6GHZ) && (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 8, 0))
	/* 6G band cap */
	cap_len = rtw_build_he_6g_band_cap_ie_by_proto(padapter, dft_proto_cap, he_6g_band_cap_ie);

	_rtw_memcpy(&sta_iface_data->he_6ghz_capa,
	            he_6g_band_cap_ie + 3, HE_6G_BAND_CAP_INFO_LEN);
#endif /* CONFIG_IEEE80211_BAND_6GHZ && (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 8, 0)) */
